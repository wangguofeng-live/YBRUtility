...
+ (NSMutableArray *) dateForCurrentDay: (NSInteger) allDay
{
    ...
    for (int i = 0; i < allDay; i ++) {
            ...
            NSDateFormatter *dateBigFormatter = [[NSDateFormatter alloc] init];
            [dateBigFormatter setDateFormat:@"yyyy年MM月dd日"];
            NSString *dateBigStr = [dateBigFormatter stringFromDate:curDate];//几年几月几号
        
           NSDateFormatter *dateSmallFormatter = [[NSDateFormatter alloc] init];
                   [dateSmallFormatter setDateFormat:@"yyyy-MM-dd"];
            NSString *dateSmallStr = [dateSmallFormatter stringFromDate:curDate];
            
            NSDateFormatter *weekFormatter = [[NSDateFormatter alloc] init];
            [weekFormatter setDateFormat:@"EEEE"];//星期几
            NSString *weekStr = [weekFormatter stringFromDate:curDate];
            
            //转换星期几为周几
            NSString *chinaStr =  [weekStr stringByReplacingOccurrencesOfString:@"星期" withString:@"周"];
           
            //组合时间
        [needArr addObject:@[dateBigStr,dateSmallStr,chinaStr]];
        }
...
- (BOOL) isSameOnlyMonthAsDate: (NSDate *) aDate
{
    NSDateComponents *components1 = [CURRENT_CALENDAR components:NSCalendarUnitYear | NSCalendarUnitMonth fromDate:self];
    NSDateComponents *components2 = [CURRENT_CALENDAR components:NSCalendarUnitYear | NSCalendarUnitMonth fromDate:aDate];
    return ((components1.month == components2.month));
...
- (BOOL) isSameMonth
{
    return [self isSameOnlyMonthAsDate:[NSDate date]];
}
...
- (BOOL) isEarlierAndSameThanDate: (NSDate *) aDate
{
    NSComparisonResult resultComp = [self compare:aDate];
    return (resultComp == NSOrderedAscending || resultComp == NSOrderedSame);
}
